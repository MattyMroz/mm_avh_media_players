...
local function create_default_menu()
    return {{
        ...
        items = {{
            ...
            value = 'script-binding uosc/open-file'
        }, {
            title = '※ Importuj Ścieżkę Napisów',
        ...
        items = {{
            ...
            value = 'script-binding uosc/playlist'
        }, {
            title = '※ Lista Wersji',
            value = 'script-binding uosc/editions'
        }, {
            title = '※ Lista Rozdziałów',
            value = 'script-binding uosc/chapters'
        }, {
            title = '※ Lista Ścieżek Wideo',
            value = 'script-binding uosc/video'
        }, {
            title = '※ Lista ścieżek dźwiękowych',
            value = 'script-binding uosc/audio'
        }, {
            title = '※ Lista ścieżek napisów',
            value = 'script-binding uosc/subtitles'
        }, {
            title = 'Losowe Odtwarzanie Utworów Na Liście Odtwarzania',
            value = 'playlist-shuffle'
        }}
    }, {
        title = '※ Zrzut ekranu',
        value = 'script-binding uosc/shot'
    }, {
        title = 'Wideo',
        items = {{
        ...
        title = 'Narzędzia',
        items = {{
            title = 'Włącz/Wyłącz Wyświetlanie Statystyki Dla Nerdów',
            value = 'script-binding stats/display-stats-toggle'
        }, {
            title = 'Wyświetl Konsolę',
            value = 'script-binding console/enable'
        }, {
            title = 'Zmień Obramowanie Okna',
            value = 'cycle border'
        }, {
            title = 'Przełącz Okno Na Tryb Zawsze Na Wierzchu',
            value = 'cycle ontop'
        }, {
            title = '※ Lista Urządzeń Wyjściowych Audio',
            value = 'script-binding uosc/audio-device'
        }, {
            title = '※ Jakość Strumieniowania',
            value = 'script-binding uosc/stream-quality'
        }, {
            title = '※ Otwórz Bieżący Folder Pliku',
            value = 'script-binding uosc/show-in-directory'
        }, {
            title = '※ Otwórz Folder Ustawień',
...
config = {
    ...
    menu_items = (function()
        ...
        local input_conf_path = mp.command_native({'expand-path',
    ...
    chapter_ranges = (function()
        ...
        if options.chapter_ranges and options.chapter_ranges ~= '' then
            for _, definition in ipairs(split(options.chapter_ranges, ' *,+ *')) do
                ...
                if name and color and name:match('^[a-zA-Z0-9_]+$') and color:match('^[a-fA-F0-9]+$') and
                    (#color == 6 or #color == 8) then
...
if options.click_threshold > 0 then
    ...
    click_timer:kill()
    mp.set_key_bindings({{'mbtn_left', function()
        last_up = mp.get_time()
    end, function()
...
mp.observe_property('pause', 'bool', create_state_setter('pause', function()
    file_end_timer:kill()
end))
...
mp.observe_property('demuxer-cache-state', 'native', function(prop, cache_state)
    ...
    if not (state.duration and
        (#cached_ranges > 0 or state.cache == 'yes' or (state.cache == 'auto' and state.is_stream))) then
    ...
    for _, range in ipairs(cached_ranges) do
        ranges[#ranges + 1] = {math.max(range['start'] or 0, 0),
...
local track_loaders = {{
    name = 'subtitles',
    hint = 'Ścieżkę Napisów',
    prop = 'sub',
    allowed_types = config.subtitle_types
}, {
    name = 'audio',
    hint = 'Ścieżkę Dźwiękową',
    prop = 'audio',
...
for _, loader in ipairs(track_loaders) do
    ...
    bind_command(menu_type, function()
        ...
        open_file_navigation_menu(path, function(path)
            ...
            type = menu_type,
            title = 'Importuj ' .. loader.hint,
            allowed_types = loader.allowed_types
        })
    end)
end
bind_command('subtitles', create_select_tracklist_type_menu_opener('Lista Ścieżek Napisów', 'sub', 'sid',
    'script-binding uosc/load-subtitles'))
bind_command('audio', create_select_tracklist_type_menu_opener('Lista Ścieżek Dźwiękowych', 'audio', 'aid',
    'script-binding uosc/load-audio'))
bind_command('video', create_select_tracklist_type_menu_opener('Lista Ścieżek Wideo', 'video', 'vid',
...
mp.add_key_binding(nil, 'shot', function()
    if Menu:is_open() then
        local bak_opt1, bak_opt2, bak_opt3, bak_opt4 = options.curtain_opacity, options.menu_opacity,
            options.menu_parent_opacity, options.pause_indicator
        ...
        if paused then
            mp.add_timeout(200 / 1000, function() -- 延迟过低可能产生闪烁
                ...
                options.curtain_opacity, options.menu_opacity, options.menu_parent_opacity = bak_opt1, bak_opt2,
                    bak_opt3
            end)
        else
...
if type(options.idle_call_menu) == 'number' then
    if options.idle_call_menu <= 2 and options.idle_call_menu > config.render_delay then
        mp.observe_property('idle-active', 'bool', function(_, value)
            if value == true then
                mp.add_timeout(options.idle_call_menu, function()
                    if Menu:is_open() then
                        return
                    else
...
if options.controls and options.controls ~= 'never' then
    require('elements/Controls'):new()
end
if itable_index_of({'left', 'right'}, options.volume) then
    require('elements/Volume'):new()
end
